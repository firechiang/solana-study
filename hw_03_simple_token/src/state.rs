...
/// Mint data.
/// 类似于代币合约信息，因为每发行一个代币就会存储一个Mint信息（简单理解就是ERC20合约信息）
/// 说明：Solana上发行代币是通过同一个合约完成的（不像Solidity每发行一个代币都要部署一个智能合约）
#[repr(C)]
...
impl Account {
    ...
    pub fn is_owned_by_system_program_or_incinerator(&self) -> bool {
        solana_program::system_program::check_id(&self.owner) || solana_program::incinerator::check_id(&self.owner)
    }
}
